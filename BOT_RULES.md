...
- `published`、`start_at`、`end_at` 一律使用 ISO 8601 完整时间格式，并显式带东八区时区：`YYYY-MM-DDTHH:mm:ss+08:00`。
- 示例：`published: '2025-10-09T23:05:06+08:00'`。
- 若仅能识别到日期、无法识别具体时分秒：
  - `start_at` 默认补 `00:00:00`；
  - `end_at` 默认补 `23:59:59`。
- 若时间无法可靠判断，`start_at` / `end_at` 填空字符串：`''`，并在工作日志标注 `time_uncertain`。
- 为避免 YAML 被错误解析，所有时间字段建议统一使用单引号包裹。
- 严禁写入不带时区的时间（如 `2025-10-09T23:05:06`），否则前端显示会发生时区偏移。
