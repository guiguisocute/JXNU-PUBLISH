...
## 语义精修与大规模迁移实战踩坑 (2026-02-11)

### 时间格式与 YAML 安全
- **强制时区**：`published`、`start_at`、`end_at` 必须显式包含 `+08:00`。
- **强制包裹**：所有 ISO 时间字符串必须使用单引号 `'` 包裹。严禁使用双引号或不包裹，以防 YAML 解析器将其误识为日期对象或在含有冒号时解析失败。
- **示例**：`end_at: '2026-01-10T23:59:59+08:00'`。

### 分类规范 (5+1) 判定准则
- **问卷填表**：关键词含“推荐表”、“汇总表”、“申报材料”、“小程序”、“统计”、“在线文档”、“填写信息”的通知一律判定为此类。
- **竞赛相关**：涉及任何级别的“赛”、“选拔”、“海选”、“挑战杯”、“辩论”、“锦标赛”。
- **志愿实习**：涉及“志愿者”、“支教”、“招募代表”、“志愿时长”。
- **二课活动**：涉及“培训”、“讲座”、“沙龙”、“分享会”、“晚会”、“艺起来”等具体参与类活动。
- **通知公告**：涉及“公示”、“决定”、“表彰”、“更正说明”、“放假/闭馆提醒”。
- **其它分类**：无法归入上述五类的兜底。

### 标签净化逻辑
- **去冗余**：全量移除 `人工智能学院` 等已通过目录或 `school_slug` 体现的地域/学院标签。
- **去重复**：严禁在 `tags` 中出现与 `category` 完全一致的词条。
- **高质量标签**：标签应体现“核心事项+具体对象”（如：`补贴发放`、`双创大赛`、`密码找回`）。

### 语义深度补全 (end_at)
- **拒绝机械脚本**：严禁简单使用脚本来判断起止时间。
- **语义优先**：必须深度阅读正文。若提及“于X月X日前提交”，`end_at` 即为该截止点。若为短期活动且未说明结束时间，方可根据活动性质合理估算（通常为 1~2 小时）。
- **截止精度**：截止日期若未说明具体小时，`end_at` 统一补齐至当日深夜 `'23:59:59+08:00'`。

### 文件并入与清理逻辑
- **并入原则**：若出现“补充通知”、“更正通知”，必须将其正文与附件并入主通知 Markdown，禁止保留多个碎片化文件。
- **发布时间继承**：并入后，目标主文件的 `published` 时间必须更新为所有关联消息中最晚（最新）的时间点，以确保在列表页的排序逻辑符合信息迭代顺序。
- **物理删除**：并入操作完成后，必须立即删除源碎片文件。

### 数据安全与恢复风险
- **读写保护**：在大规模脚本重写（如正则替换标签或时间）时，Python 的 `open(path, 'w')` 若中途崩溃可能导致文件清空（0 字节）。
- **防御策略**：建议先读取全部内容到内存，处理完成后一次性写入；或采用“写临时文件 -> 重命名覆盖”的原子操作。
