...
- Bot 持续监听与归归私聊窗口中的转发信息，及“Telegram 聊天记录json + 文件”的压缩包输入。
- 检测到新通知且听到归归开始分析的口号后，立即触发增量处理，不堆积到单一时间点。
- 每次增量处理控制批大小，避免上下文爆炸。
...
- 允许外链附件（http/https）。
- 若消息包含图片，图片文件必须先落盘到 `content/img/...`，再用站内路径 `/img/...` 引用，禁止保留本地临时路径（如 `file://`）。
